...
export type AgentResponse =
  ...
  | { type: 'tool_call'; tool: ToolName; args: Record<string, unknown>; result?: string; fileContent?: string[] }
...
const HARDCODED_RESPONSES: AgentResponse[] = [
  { type: 'text', content: "I'll help you implement this feature. Let me start by exploring the project structure." },
  { type: 'tool_call', tool: 'list_dir', args: { path: '.' }, result: 'src/\npackage.json\ntsconfig.json\nmock-file.txt' },
  { type: 'text', content: "Now let me read the main configuration file to understand the project setup." },
  { type: 'tool_call', tool: 'read_file', args: { path: 'package.json' }, result: '{\n  "name": "my-app",\n  "version": "1.0.0"\n}' },
  { type: 'tool_call', tool: 'list_dir', args: { path: 'src' }, result: 'components/\nApp.tsx\nmain.tsx' },
  { type: 'text', content: "I see the project structure. Let me add the initial implementation to the mock file." },
  { 
    type: 'tool_call', 
    tool: 'edit_file', 
  ...
  { type: 'text', content: "I've added the basic structure. Now let me expand it with more functionality." },
  { 
    type: 'tool_call', 
    tool: 'edit_file', 
  ...
  { type: 'text', content: "Now let me run the tests to make sure everything works." },
  { type: 'tool_call', tool: 'run_test', args: { testFile: 'src/tests/mock-file.test.tsx' }, result: 'PASS: All tests passed (3/3)' },
  { type: 'text', content: "All tests pass. The feature has been successfully implemented. Here's a summary of what I did:\n\n1. Explored the project structure\n2. Read the configuration\n3. Added initial implementation (3 lines)\n4. Expanded with more functionality (5 more lines)\n5. Ran tests to verify\n\nLet me know if you need any changes!" },
]
...
function generateFileContent(baseLineCount: number, linesAdded: number): string[] {
...
export async function* sendMessage(
  ...
  forceError?: boolean,
  signal?: AbortSignal
): AsyncGenerator<{ response: AgentResponse; fileContent?: string[]; newIndex: number }> {
  ...
  while (currentIndex < HARDCODED_RESPONSES.length) {
    ...
    const result: { response: AgentResponse; fileContent?: string[]; newIndex: number } = { 
      response, 
      newIndex: currentIndex + 1 
    }
    
    if (response.type === 'tool_call' && response.tool === 'edit_file') {
      const linesAdded = (response.args as { linesAdded?: number }).linesAdded || 0
    ...
    if (currentIndex < HARDCODED_RESPONSES.length) {
      await sleep(randomDelay(MIN_SUBSEQUENT_DELAY, MAX_SUBSEQUENT_DELAY), signal)
    }
  }
}
...
export async function rollbackToMessage(
  messageId: string,
  messages: { id: string; type: string; toolName?: string; status?: string; fileContent?: string[] }[],
  signal?: AbortSignal
  ...
  for (let i = relevantMessages.length - 1; i >= 0; i--) {
    ...
    if (msg.type === 'tool_operation' && msg.toolName === 'edit_file' && msg.status === 'completed' && msg.fileContent) {
...
export function getToolTarget(tool: ToolName, args: Record<string, unknown>): string {
  switch (tool) {
    case 'list_dir':
