...
export function MessageList({ messages, isAgentWorking = false }: MessageListProps) {
  const { setContainer, hasUnseenMessages, scrollToBottom, onContentAdded, onUserMessageSent } = useAutoScroll({ threshold: 100 })
      ...
      case 'agent_message': {
        ...
        return (
          <div className="mx-3 min-w-0 overflow-hidden">
            ...
            <FeedbackForm messageId={message.id} currentFeedback={agentMsg.feedback} />
          </div>
        )
      }
      case 'user':
        ...
        elements.push(
          ...
          >
            {toolCallGroup.map((msg) => (
              <div key={msg.id} id={`message-${msg.id}`}>{renderMessage(msg)}</div>
            ))}
          </div>
        )
        toolCallGroup = []
  ...
  if (messages.length === 0) {
    return (
      <div className="flex flex-1 justify-center items-center p-4">
        <p className="text-muted-foreground text-sm text-center">Start a conversation...</p>
      </div>
    )
  }
  ...
  return (
    <div className="relative flex-1 w-full min-w-0 min-h-0 flex flex-col overflow-hidden">
      <ScrollArea ref={scrollAreaRef} className="flex-1 w-full min-w-0 min-h-0">
        <div className="px-3 pt-4 pb-40 space-y-3 flex flex-col overflow-hidden">{renderMessages()}</div>
      </ScrollArea>
      <ScrollToBottomButton visible={hasUnseenMessages} onClick={scrollToBottom} />
    </div>
  )
}
