  ...
  return (
    <Collapsible.Root open={isOpen} onOpenChange={setIsOpen} className="min-w-0">
      ...
      >
        ...
        {description && (
          <span
            className={cn("shrink-[99999] min-w-0 truncate ml-1.5 opacity-70", status === 'running' && "shimmer-text")}
            style={status === 'running' ? { '--shimmer-delay': shimmerDelay } as React.CSSProperties : undefined}
          >
            {description}
          </span>
        )}
        ...
        {hasExpandableContent && (
          <span
            className={cn(
              ...
              isOpen ? 'w-3 opacity-100' : 'w-0 opacity-0 group-hover:w-3 group-hover:opacity-100'
            )}
          >
      ...
      {hasExpandableContent && (
        <Collapsible.Content
          className="py-1.5 pl-5 text-muted-foreground text-xs font-mono bg-muted/30 rounded mt-1 overflow-x-auto max-h-64 overflow-y-auto"
        >
